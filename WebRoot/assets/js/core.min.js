...
function actionFormate(data, isShowAlter, successFn, errorFn) {
	if (data.type == "SUCCESS") {
		if ($.isFunction(successFn))
			successFn(data.type, data.msg, data.data);
	} else {
		if ($.isFunction(errorFn))
			errorFn(data.type, data.msg, data.data);
	}
	if (isShowAlter){
		showMsg(data.msg);
	}
...
function getShareData(key){
	return $(".main").data(key);
}

function showMsg(context, title) {
	var myModal = new jBox('Notice', {
		attributes : {
			x : 'right',
			y : 'top'
		},
		...
		onInit : function() {
			...
			this.options.title = !title ? "提示：" : title;
...
function initPageDom() {
	...
	$('.btn-minimize').click(
			function(t) {
				t.preventDefault();
				var n = $(this).parent().parent().next('.panel-body');
				n.is(':visible') ? $('i', $(this)).removeClass('fa-chevron-up')
						.addClass('fa-chevron-down') : $('i', $(this))
						.removeClass('fa-chevron-down').addClass(
								'fa-chevron-up');
...
$(document).ready(
		function(e) {
			e('ul.nav-sidebar').find('a').each(
					function() {
						if (e(e(this))[0].href == String(window.location)) {
							e(this).parent().addClass('active');
							e(this).parents('ul').add(this).each(
									function() {
										e(this).show().parent().addClass(
												'opened').addClass(
												'nav-expanded');
									})
						}
					});
			e('.nav-sidebar a').click(
					function(t) {
						setTimeout(function() {
			...
			e('#main-menu-toggle').click(
					function() {
						e('body').hasClass('sidebar-hide') ? e('body')
								.removeClass('sidebar-hide') : e('body')
